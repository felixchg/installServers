...
cat << END > /root/.screenrc
# Start message (turn off screen welcome message)
startup_message off

# to avoid visual bell
vbell off

# Set hardstatus always on (add status line)
...
# Add alt+z & alt+x to switch screen
bindkey ^[z prev
bindkey ^[x next
# for putty - use Ctrl + <- & Ctrl + -> to switch screen
...
# How to get keymap
# https://www.puritys.me/filemanage/blog_files/keys.pl

shell -$SHELL
END

mkdir -p /root/.vim/colors
cat << END > /root/.vim/colors/badwolf.vim
"      _               _                 _  __
